...
import { 
  ...
  PlusCircle, ShoppingBag, Tag, Search, X, Hammer
} from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => { 
    setMounted(true); 
    checkUser();
    // Подписка на сообщения для реалтайма (опционально)
  }, []);
  ...
  if (profile?.is_blocked) return (
    <div style={{background:'#000',height:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',color:THEME.red}}>
      <Lock size={64}/><h1>ACCOUNT_LOCKED</h1>
      <p>До: {new Date(profile.banned_until).toLocaleString()}</p>
    </div>
  );
    ...
    <main style={{ background: THEME.bg, color: THEME.text, height: '100vh', display: 'flex', fontFamily: 'monospace' }}>
      ...
      <nav style={{ width: '260px', background: THEME.sidebar, borderRight: `1px solid ${THEME.border}`, padding: '20px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '5px' }}>
          ...
          {profile?.is_admin && (
            <button onClick={() => setActiveTab("admin")} style={{ ...btnTab(activeTab === "admin"), color: THEME.red, marginTop: '20px' }}><ShieldAlert size={18}/> АДМИНКА</button>
          )}
        </div>
        ...
        {activeTab === "profile" && (
          <div>
            <div style={{ height: '180px', background: `url(${profile?.banner_url || ''})`, backgroundColor: '#111', backgroundSize: 'cover', borderRadius: '20px', position: 'relative', border: `1px solid ${THEME.border}` }}>
              <img src={profile?.avatar_url || ''} style={{ width: '80px', height: '80px', borderRadius: '50%', border: `4px solid ${THEME.bg}`, position: 'absolute', bottom: '-40px', left: '30px', objectFit: 'cover' }} alt=""/>
            </div>
        ...
        {activeTab === "chat" && (
          <div style={{ display: 'flex', gap: '20px', height: '100%' }}>
            <div style={{ width: '250px', borderRight: `1px solid ${THEME.border}` }}>
              <h3>ДРУЗЬЯ</h3>
              {friends.map(f => (
                <div key={f.id} onClick={() => openChat(f)} style={{ padding: '12px', borderRadius: '10px', background: selectedChat?.id === f.id ? THEME.card : 'none', cursor: 'pointer' }}>{f.username}</div>
              ))}
            </div>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              {selectedChat ? (
                <>
                  <div style={{paddingBottom:'10px', borderBottom:`1px solid ${THEME.border}`}}>{selectedChat.username}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
            {storeGifts.map(g => (
              <div key={g.id} style={{ background: THEME.card, padding: '15px', borderRadius: '15px', border: `1px solid ${THEME.border}` }}>
                ...
                <div style={{fontSize:'11px', color:THEME.muted}}>{g.is_nft ? `NFT: ${g.current_supply}/${g.max_supply}` : 'Подарок (∞)'}</div>
            ...
            {marketItems.map(m => (
              <div key={m.id} style={{ background: m.metadata?.bg || THEME.card, padding: '15px', borderRadius: '15px', border: `1px solid ${THEME.border}` }}>
                ...
                <div style={{display:'flex', justifyContent:'space-between', marginTop:'15px', alignItems:'center'}}>
                  ...
                  {m.user_id !== profile.id && <button onClick={() => alert("Покупка на рынке в процессе разработки")} style={btnAction}>КУПИТЬ</button>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{display:'flex', gap:'40px'}}>
            <div style={{flex: 1}}>
              ...
              <div style={{display:'flex', flexDirection:'column', gap:'10px', background:THEME.card, padding:'20px', borderRadius:'15px'}}>
                ...
                <label><input type="checkbox" onChange={e=>setNewGift({...newGift, is_nft: e.target.checked})}/> Это NFT</label>
                {newGift.is_nft && <input type="number" placeholder="Лимит" onChange={e=>setNewGift({...newGift, max_supply: parseInt(e.target.value)})} style={inputStyle}/>}
                <button onClick={createItem} style={{background:THEME.success, border:'none', color:'#fff', padding:'10px', borderRadius:'10px', cursor:'pointer'}}>ОПУБЛИКОВАТЬ</button>
              </div>
            </div>
            <div style={{flex: 1}}>
              <h3>ПОЛЬЗОВАТЕЛИ</h3>
              {allUsers.map(u => (
                <div key={u.id} style={{padding:'10px', background:THEME.card, borderRadius:'10px', marginBottom:'5px', display:'flex', justifyContent:'space-between'}}>
                  <span>{u.username}</span>
                  <div style={{display:'flex', gap:'5px'}}>
                    <button onClick={() => banUser(u.id)} style={{background:THEME.red, border:'none', color:'#fff', padding:'5px', borderRadius:'5px'}}><Ban size={14}/></button>
                    <button onClick={() => confirm("Удалить?") && supabase.from('profiles').delete().eq('id', u.id)} style={{background:'#333', border:'none', color:'#fff', padding:'5px', borderRadius:'5px'}}><Trash2 size={14}/></button>
                  </div>
                </div>
              ))}
            </div>
...
const btnTab = (active: boolean) => ({
  background: active ? '#111' : 'none', border: 'none', color: active ? '#fff' : '#777', padding: '12px', borderRadius: '10px', textAlign: 'left' as const, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px'
});
const inputStyle = { width: '100%', background: '#000', border: `1px solid ${THEME.border}`, color: '#fff', padding: '10px', borderRadius: '10px' };
const btnAction = { background: '#fff', color: '#000', border: 'none', padding: '6px 15px', borderRadius: '6px', fontWeight: 'bold' as const, cursor: 'pointer', fontSize: '11px' };
